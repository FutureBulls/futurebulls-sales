    ...
    <>
      <li>
        <Link
          href={item.route}
          onClick={handleClick}
          className={`${pageName === item.label.toLowerCase() ? "bg-gradient-to-r from-primary/10 to-secondary/10 text-primary border-l-4 border-primary dark:bg-gradient-to-r dark:from-primary/20 dark:to-secondary/20 dark:text-white dark:border-primary" : "text-gray-6 hover:bg-gradient-to-r hover:from-gray-1 hover:to-gray-2 hover:text-dark dark:text-gray-5 dark:hover:bg-gradient-to-r dark:hover:from-dark-3 dark:hover:to-dark-4 dark:hover:text-white"} group relative flex items-center gap-3 rounded-lg px-4 py-3 font-medium duration-300 ease-in-out transition-all hover:shadow-sm`}
        >
          {item.icon}
          {item.label}
          {item.message && (
